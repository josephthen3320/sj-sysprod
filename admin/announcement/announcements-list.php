...
while ($a = $result->fetch_assoc()) {
    ...
    echo "<div class='w3-bar-item w3-leftbar w3-border-{$colour} w3-hover-light-grey' style='padding: 16px 40px 16px 20px;' onclick='openPopupURL2(\"index.php\")'>";
    echo "<div class='w3-container w3-cell w3-cell-middle'>";
    echo "<i class='fa-solid {$icon} fa-2xl w3-text-{$colour}'></i>";
    echo "</div>";
    echo "<div class='w3-container w3-cell w3-cell-middle w3-display-container' style='width:100%;'>";
    echo "<span class='w3-small w3-display-topright'><b>{$a['timestamp']}</b></span>";
    echo "<span style='font-weight: bold;'>{$a['subject']}</span> <br>";
    echo "<span>{$a['details']}</span>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
}
