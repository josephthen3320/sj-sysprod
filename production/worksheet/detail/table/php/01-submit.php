...
$sql = "SELECT * FROM qty_size WHERE worksheet_id = '$worksheet_id' AND x_loc = $columnIndex AND y_loc = $rowIndex";
$result = $conn->query($sql);

if ($result !== false && $result->num_rows > 0) {
    // Cell already has a value, update the existing value in the table
    $updateSql = "UPDATE qty_size SET value = '$value' WHERE worksheet_id = '$worksheet_id' AND x_loc = $columnIndex AND y_loc = $rowIndex";
    ...
    $insertSql = "INSERT INTO qty_size (worksheet_id, x_loc, y_loc, value) VALUES ('$worksheet_id', $columnIndex, $rowIndex, '$value')";
