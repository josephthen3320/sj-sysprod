...
<div class="w3-bar" style="display: flex; justify-content: center;">
    <span class="w3-bar-item"><b>SIZE SPEC REGULAR</b></span>
</div>
<div class="w3-tiny">
    <table class="tg" style="width: 100%;" id="tableSizeSpec" data-worksheet-id="<?= $worksheet_id ?>">
        <thead>
        <tr>
            <th class="tg-ncfi w3-tiny">SIZE</th>
            <th class="tg-ncfi w3-tiny">Chest</th>
            <th class="tg-ncfi w3-tiny">PANJANG BADAN <br>(HPS)</th>
            <th class="tg-ncfi w3-tiny">PANJANG<br>TANGAN</th>
            <th class="tg-ncfi w3-tiny">PLAKET</th>
        </tr>
        </thead>
        <tbody>
        <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const cells = document.querySelectorAll('#tableSizeSpec tbody td[contenteditable]');

        // Loop through each cell and attach event listeners
        cells.forEach(function(cell) {
            cell.addEventListener('input', function() {
                const worksheetId = document.querySelector('#tableSizeSpec').dataset.worksheetId;
                ...
                xhr.open('POST', 'table/php/02-submit.php');
                    ...
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 200) {
                            ...
                            if (response.hasValue) {
                                ...
                                document.querySelector(`#tableSizeSpec tbody tr:nth-child(${rowIndex + 1}) td:nth-child(${columnIndex + 1})`).textContent = value;
                            } else {
...
<?php include "php/02-fetch-content.php";