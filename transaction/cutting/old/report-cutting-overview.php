    ...
    <div class="w3-row">
        ...
        <div class="form-container">
            ...
            <select class="w3-border w3-select" id="cmt_filter" name="cmt_filter">
                ...
                $cmtQuery = "SELECT cmt_id, cmt_name FROM cmt WHERE cmt_type = 'CT1'";
...
<table class="w3-table w3-table-all">
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $modelname = getArticleById($worksheet['article_id'])['model_name'];

        echo "<td>{$row['date_in']}</td>";
        echo "<td class='w3-right-align'>{$row['qty_out']}</td>";
        echo "<td>{$cmtname}</td>";
