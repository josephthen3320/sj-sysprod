...
<div class="w3-row w3-hide-small w3-small">
    <div class='w3-col w3-row l12 m12 s12 w3-white w3-border w3-border-light-gray w3-center'>
        <div class="w3-col l3 m3 s12 jt-padding-tb" style="font-weight: bold;">
            ...
            <div class="w3-col l5 m5 s12 jt-padding-tb" style="font-weight: bold;">Finishing ID</div>
        ...
        <div class="w3-col l2 m1 s12 jt-padding-tb w3-row" style="font-weight: bold;">
    ...
    <?php
        ...
        while ($ct = $ct_data->fetch_assoc()) {
            ...
            // Finishing Out Data
            $finishingOutData = fetchFinishingOutRecords($ct['finishing_id']);
            $detailparam = '';
            $chevron = "";

            if ($finishingOutData->num_rows > 0) {
                $detailparam = "class='w3-hover-text-red' style='cursor: pointer;' onclick='toggleDetail(\"{$ct['finishing_id']}\")'";
            ...
            echo "<div class='w3-col l3 m3 s12 jt-padding-tb-8 w3-row'>";
                ...
                echo "<div class='w3-col l5 m5 s12 jt-padding-tb-8'>{$ct['finishing_id']}</div>";
...
*/
            echo "<div class='w3-col l2 m1 s12 jt-padding-tb-8 w3-row'>";
                ...
                if (checkSuratJalanExistsByTransactionId($ct['finishing_id'])) {
                    ...
                    if (checkSuratJalanPrinted($ct['finishing_id'])) {
                ...
                if (getWorksheetPosition($ct['worksheet_id']) == 8) {
                    if (($ct['qty_out'] + $ct['qty_missing']) == $ct['qty_in']) {
                        if (in_array($role, [0,1,2,3,7])) {
                            echo "<button class='w3-button w3-red' onclick='openPopupURL2(\"sendDialog.php?w={$ct['worksheet_id']}&i={$ct['id']}&pi={$ct['finishing_id']}&q={$ct['qty_out']}\", \"sendto\", 500, 400)'><i class=\"fa-solid fa-arrow-right-from-arc\"></i></button>";
                        }
                    }
            ...
            echo "<div class='w3-col w3-row l12 m12 s12 w3-pale-blue w3-center {$ct['finishing_id']}' style='display: none; padding-top:8px; padding-bottom: 24px;'>";
            while ($out = $finishingOutData->fetch_assoc()) {
                ...
                $timestamp = strtotime($out['timestamp']);
                $datestamp = date("Y-m-d", $timestamp);

                echo "<div class='w3-col l1 m1 jt-padding-tb-8'>{$description}</div>";
                echo "<div class='w3-col l1 m1 jt-padding-tb-8'>{$datestamp}</div>";
...
<!--table class="w3-table w3-table-all">
    ...
    <tr class="">
        ...
        <th  style="text-align: center; vertical-align: middle;">Finishing No.</th>
    ...
    <tbody>
        ...
        <?php
            ...
            while ($ct = $ct_data->fetch_assoc()) {
                ...
                ++$i;
                //echo "<tr>";
                echo "<tr onclick='toggleDetail(\"{$ct['finishing_id']}\")' style='cursor: pointer;'>";
                echo "<td>{$i}</td>";
                ...
                echo "</tr>";
                // Detail of finishing out
                $finishingOutData = fetchFinishingOutRecords($ct['finishing_id']);


                while ($out = $finishingOutData->fetch_assoc()) {
                    echo "<tr class='w3-pale-blue w3-small {$ct['finishing_id']}' id='detail-{$ct['finishing_id']}'>";
                    echo "<td colspan='5'>&nbsp;</td>";
                    echo "<td>{$out['qty_out']}</td>";
                    echo "<td>{$out['qty_missing']}</td>";
                    echo "<td>&nbsp;</td>";
                    echo "<td>{$out['timestamp']}</td>";
...
<script>
    function toggleDetail(finishing_id) {
        var details = document.getElementsByClassName(finishing_id);
