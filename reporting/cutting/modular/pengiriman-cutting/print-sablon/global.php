...
// Step 2: Retrieve data from the "print-sablon" table
...
<head>
    ...
    <title>Global Print/Sablon Report</title>
...
<table class="w3-table-all w3-small">
    ...
    $sablonCMTSQL = "SELECT cmt_id FROM cmt WHERE cmt_type = 'CT2'";
    $sablonResult = $connProduction->query($sablonCMTSQL);
    $cmtArray = array();

    while ($sew = $sablonResult->fetch_assoc()) {
        $cmt_id = $sew['cmt_id'];
        $cmtArray[$cmt_id] = array('A' => 0, 'B' => 0, 'C' => 0, 'Lain' => 0);
    }

    // Reset Result
    $sql = "SELECT * FROM print_sablon";
    $result = $conn->query($sql);
